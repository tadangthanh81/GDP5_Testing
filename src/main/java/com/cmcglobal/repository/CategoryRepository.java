...
public interface CategoryRepository extends JpaRepository<Category, Integer> {
	...
	@Query("select c from Category c where c.categoryName like ?1 and c.status = 1")
	...
	@Query("select count(category_id) from Category where status=1")
	String questionCategorySum();
	
	@Query("select c FROM Category c where status=1")
	List<Category> getListCategory();
	
	@Query("select count(category_id) from Category c where c.categoryName like ?1 and c.status = 1")
